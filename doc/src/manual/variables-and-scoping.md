...
--------- | ---------  |---------------
files, REPL (i.e. `Main` module), [`module`](@ref), [`baremodule`](@ref) | global | global
[`struct`](@ref) | local (soft) | global
[`for`](@ref), [`while`](@ref), [`try`](@ref try) | local (soft) | global or local
[`macro`](@ref) | local (hard) | global
[`let`](@ref), functions, comprehensions | local (hard) | global or local
...
dot-notation, i.e. each module is a so-called *namespace* which means it is a first-class data
structure associating names with values. Note that while variable bindings can be read externally,
they only be changed within the module to which they belong. As an escape hatch, you can always
evaluate code inside that module to modify a variable; this guarantees that in code that does not
call `eval` module bindings cannot be modified externally.
...
scope construct (i.e. within a let block, function or macro body, or comprehension), a new local
named `x` is created in the scope of the assignment;
...
function sum_to(n)
    s = 0 # new local
    for i = 1:n
        s = s + i # assigns existing local
    end
    return s # same local
end
```

As in the previous example, the first assignment to `t` at the top of `sum_to` causes `s` to be a
...
Since there global `x` is not defined when the for loop is evaluated, the first clause of the soft
...
function. However, it has some down sides. First, it's quite a complex behavior: many people over
the years were confused about this behavior and complained that it was complicated and hard to
understand and to explain. Fair point. Second, and arguably worse, is that it's bad for programming
"at scale." When you see a small piece of code in one place like this, it's quite clear what's going
on:
...
# or maybe back in the one where `x = 123` was

y = x + 234
```

Here it far less clear what should happen. Since `x + "hello"` is a method error, it seems probable
that the intention is for `x` to be local to the `for` loop, but the types of runtime values and
what methods happen to be defined cannot determine the scope of variables. With the Julia 0.6
behavior, it's especially concerning that someone might have written the for loop first and it
worked just fine, but then when someone adds a new global far away, possibly in a different file,
the code suddenly changes meaning and either breaks noisily as in this case, or worse still, might
break silently and just do the wrong thing. This kind of "spooky action at a distance" is the kind
of thing good programming language designs should prevent.
...
scope entirely and eliminated the spooky action potential that having a global by a certain name
previously had. We uncovered and fixed a log of bugs in the process. And there was much rejoicing!
Well, no, not really. Because the people were angry that they now had to write:
...
2. Beginners will write this without the `global` and have no idea why their code doesn't work—the
   error that they get is that `s` is undefined, which does not seem to enlighten anyone who happens
   to make this mistake.

As of Julia 1.5, the new behavior is that the code works without the `global` annotation in
interactive contexts like the REPL or Jupyter notebooks (just like Julia 0.6) and in files and other
non-interactive contexts, it prints this warning:
